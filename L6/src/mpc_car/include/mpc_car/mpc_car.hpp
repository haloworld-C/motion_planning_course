...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    Ad_ << 0, 0, -v * std::sin(phi), std::cos(phi),
        0, 0, v * std::cos(phi), std::sin(phi),
        0, 0, 0, std::tan(delta) / ll_;
    Bd_ << 0, 0,
        0, 0,
        0, v / (ll_ * std::pow(std::cos(delta), 2)),
        0, 1;
    gd_ << v * std::sin(phi) * phi,
        -v * std::cos(phi) * phi,
        -v / (ll_ *std::pow(std::cos(delta), 2)) * delta,
        0;
    return;
  }
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    P_.resize(m * N_, m * N_);//N_预测步长
    q_.resize(m * N_, 1);
    Qx_.resize(n * N_, n * N_);
    // stage cost
    Qx_.setIdentity();
    for (int i = 1; i < N_; ++i) {
      Qx_.coeffRef(i * n - 2, i * n - 2) = rho_; // phi weight
      Qx_.coeffRef(i * n - 1, i * n - 1) = 0; // v weight
    }
    // terminal cost
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    AA.setZero(n * N_, n);
    gg.setZero(n * N_, 1);
    double s0 = s_.findS(x0.head(2)); // 参考路径上的第一个点
    double phi, v, delta;
